...
def compose_message(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Save the new_message with updated attachments
            new_message.save()

            # Retrieve selected email ids
            ...
            # Attachments
            for attachment in attachments:
                # Access the File object associated with the FieldFile
                file_obj = attachment.file.file  # Adjust this based on your model structure
                
                # Attempt to get content_type using mimetype attribute
                content_type = getattr(file_obj, 'mimetype', None)
                
                if not content_type:
                    # If mimetype attribute is not available, use mimetypes module to guess content type
            ...
            return redirect('manageforms')  # Replace with the URL you want to redirect to on successful save

    # If it's a GET request, create a form with existing data if available
    existing_message = ComposeMessage.objects.first()
